...
func InsertItemSold() MetricSoldItem {
	randNameAndLocation := generateRandomValue(1, 10)
	randTotalWeight := generateRandomValue(100, 300)
	randSoldWeight := generateRandomValue(1, randTotalWeight)
	randDonateWeight := generateRandomValue(1, randTotalWeight)
	...
	randEthylene := generateRandomValue(50, 400)
	randCarbon := generateRandomValue(400, 1200)

	t := strconv.Itoa(int(sku))

	item := MetricSoldItem{
		ItemID:        generateNewUUID(),
		SaleID:        generateNewUUID(),
		SKU:           t,
		Name:          name,
		Lot:           lot,
		TotalWeight:   float64(randTotalWeight),
		SoldWeight:    float64(randSoldWeight),
		DonateWeight:  float64(randDonateWeight),
		WasteWeight:   float64(randWasteWeight),
		Timestamp:     timestamp,
		Ethylene:      float64(randEthylene),
		CarbonDioxide: float64(randCarbon),
	}
