...
var _ = Describe("Mongo service test", func() {
	var (
		...
		item1 MetricSoldItem
		item2 MetricSoldItem
	)
	...
	BeforeEach(func() {
		...
		createTestDatabase("mtest", &MetricSoldItem{})
	...
	It("Check avgSold for inserted records", func() {
		...
		for _, v := range avgSoldReport {
			...
			Expect(assertOK).To(BeTrue())
			log.Println(avgSold)
			if avgSold == item1.SoldWeight {
				Expect(avgSold).To(Equal(item1.SoldWeight))
				log.Println(item1.SoldWeight)
			} else {
				Expect(avgSold).To(Equal(item2.SoldWeight))
				log.Println(item2.SoldWeight)
			}
	...
	It("Insert into report the avgSold results", func() {
		...
		for _, v := range findResults {
			...
			Expect(m.ReportResult[0].SoldWeight).To(Equal(101))
		}
	})
