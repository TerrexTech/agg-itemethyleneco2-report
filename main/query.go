...
func Query(itemSoldColl *mongo.Collection, reportColl *mongo.Collection, event *model.Event) *model.KafkaResponse {
	...
	if err != nil {
		err = errors.Wrap(err, "Query: Error while unmarshalling Event-data - ItemSoldMetricReport")
		logger.E(tlog.Entry{
	...
	if &filter == nil {
		...
		err = errors.Wrap(err, "Query left blank - ItemSoldMetricReport")
		logger.E(tlog.Entry{
			Description: err.Error(),
			ErrorCode:   1,
		}, filter)
	...
	if err != nil {
		err = errors.Wrap(err, "Error getting results from ItemSoldMetricReport")
		logger.E(tlog.Entry{
			Description: err.Error(),
			ErrorCode:   1,
		}, filter)
	...
	if len(avgSoldReport) < 1 {
		err = errors.New("Error: No result found from agg_itemethylene collection - Function = ItemSoldMetricReport")
		logger.E(tlog.Entry{
			Description: err.Error(),
			ErrorCode:   1,
		}, reportAgg)
